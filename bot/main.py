...
def build_scheduler(cfg, pub: Publisher, store: Store) -> Scheduler:
    ...
    # 유튜브 루틴 자극 (하루 2회) — 한때 4회였으나 발행량을 절반으로 줄이면서
    # 아침/저녁만 남겼다. 12:30(quickfix)/15:00(quickfix_weekend)/18:00(hyrox·quiz)/
    # 20:00(격주 reset_day)/21:00(weekly)/22:00(checkin) 과 안 겹치는 시각이다.
    ...
    # 파라님 9/25 「카드는 새벽 5시로」(처음 6시 → 5시) — 목표·몸무게·마라톤 카드는 전부 05:00
    sched.add(DailyJob(name="weight", fn=pub.weight_card, hour=5, minute=0, weekdays=(6,)))
    sched.add(DailyJob(name="goals", fn=pub.goals_card, hour=5, minute=0, weekdays=(0,)))
