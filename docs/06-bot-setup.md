...
| 시각 | 발행 | 비고 |
|---|---|---|
| **1시간마다 16~21시** | 자극 드롭 (영상 / 사진 / 명언 카드) | 오후에만, 나머지 시간은 쉰다 |
| **06:30** | 오늘의 3단 미션 | 요일 테마 자동 |
| **07:00 · 19:30** | 유튜브 루틴 | 유튜브 채널 운영 독려, 매일 2회 |
...
하루 총 **12~14건** 발행된다 (자극 드롭만 6건). 혼자 쓰는 비공개 채널이면 이 정도가 적당하고,
더 줄이고 싶으면 `PHOTO_INTERVAL_MINUTES` 를 120~180 으로 늘리거나
`PHOTO_START_HOUR`/`PHOTO_END_HOUR` 범위를 더 좁히면 된다.
...
> 예약 실행은 GitHub 이 혼잡할 때 몇 시간까지도 늦게 뜬다 (실측상 최대 12시간).
> 그래서 `JOB_GRACE_MINUTES=480` 으로 지연을 흡수한다. 06:30 미션이 늦게 나갈 수는 있어도
> 통째로 누락되지는 않는다.
...
1시간마다 아래 세 가지를 번갈아 낸다. 앞의 것이 안 되면 다음으로 넘어가고,
