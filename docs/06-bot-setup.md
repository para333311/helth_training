...
| 시각 | 발행 | 비고 |
|---|---|---|
| **매시 정각 07~22시** | 자극 드롭 (영상 / 사진 / 명언 카드) | 밤에는 쉰다 |
| **07:30** | 오늘의 3단 미션 | 요일 테마 자동 |
| **12:30** (평일) / **15:00** (주말) | 귀찮음 응급처치 · 270kcal 카드 | 수·일은 kcal 카드 |
| **18:00** | 하이록스 WOD | 크로스핏 보드 스타일, 매일 |
| **금 18:00** | 근육 상식 퀴즈 | 하이록스 WOD 와 같은 시각(금요일만 겹침) |
| **22:00** | 오운완 체크인 | |
| **일 21:00** | 주간 결산 | |
| **격주 일 20:00** | 리셋 데이 | |

하루 총 **20~21건** 발행된다. 혼자 쓰는 비공개 채널이면 이 정도가 적당하고,
사람이 늘면 `PHOTO_INTERVAL_MINUTES` 를 120~180 으로 늘리는 걸 권장한다.
...
매시 정각에 아래 세 가지를 번갈아 낸다. 앞의 것이 안 되면 다음으로 넘어가고,
...
하이록스 WOD 는 요일과 무관하게 재고 전체를 다 쓸 때까지 같은 게 반복되지 않는다 (명언 카드와 동일한 순환 방식). 재고가 지겨워지면 `hyrox.json` 에 같은 형식(`name`/`style`/`green`/`yellow`/`red`/`note`)으로 추가하면 된다.
