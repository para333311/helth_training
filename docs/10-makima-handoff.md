...
| 4 | 마키마가 하루 4번 코치로 채널에 말한다 | 07:40 / 19:00 / 22:20 / (일)21:20 |
...
| `openclaw/jobs.md` | 잡 4개 명세 + 헬스봇과 겹치지 않는 시간표 |
...
`-DryRun` 출력이 기대와 맞는지 확인한다 — 잡 4개, 07:40 / 19:00 / 22:20 / 일 21:20,
...
`-List` 에 `coach:morning`, `coach:condition`, `coach:night`, `coach:weekly` 4개가
보이고 **기존 비서 잡이 그대로 남아 있어야** 한다.
