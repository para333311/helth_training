...
│    └─ coach: cron 3개 → 코치. 하루 3번만 깨어난다 (컨디션은 /condition DM 명령으로 대체)
...
- 마키마 코치 턴 = **하루 3회로 상한** (컨디션 cron 은 기본 미등록 — 아래 참조)
...
### 마키마 코치 잡 (`secretary1/openclaw/register-cron.ps1` 이 등록, 기본 3개)
...
**`coach:condition`(19:00, 컨디션 1~5 묻기)은 기본으로 등록하지 않는다.**
이 채널은 구독자가 답장할 수 없는 방송 전용 비공개 채널이라, 채널에 질문을
올려도 답을 받을 방법이 없다. 게다가 `--session isolated` cron 세션은 답이
올 때까지 기다려주지 않고 그 자리에서 끝난다. 대신 헬스봇에 `/condition 1~5`
DM 명령을 추가해서 운영자가 직접 기록한다 — `--brief` 의 `condition_stale`/
`needs_rest` 는 그 값을 그대로 읽으므로 나머지 코치 판단엔 지장이 없다.
원안대로 쓰려면 `register-cron.ps1 -IncludeCondition` 이 있지만, 위 이유로
질문만 나가고 답을 못 받으니 권하지 않는다.
...
| 4 | 마키마가 하루 3번 코치로 말한다 | 07:40 / 22:20 / (일)21:20 (컨디션은 /condition DM) |
